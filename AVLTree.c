...
node * new_node(int addy, char *line){
    ...
    temp->address = addy;
    temp->instruction = malloc(sizeof(line));
    strcpy(temp->instruction, line);
    temp->height = 1;
    temp->left = NULL;
    temp->right = NULL;
    return (temp);
...
node * insert_string(node *N, int addy, char *line){    
    if(N == NULL)
        return (new_node(addy, line));

    if(0 > strcmp(line, N->instruction))
        N->left = insert(N->left, addy, line);
    else if(0 < strcmp(line, N->instruction))
        N->right = insert(N->right, addy, line);
    else
        return N;
    ...
    // left left 
    if( balance > 1 && 0 > strcmp(line, N->instruction))
        return right_rotate(N);
    // right right
    if( balance < -1 && 0 < strcmp(line, N->instruction))
        return left_rotate(N);
